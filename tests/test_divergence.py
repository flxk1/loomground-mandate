...
from loomground_mandate import (
...
def test_the_knowledge_store_is_reached_through_the_port_not_imported():
    ...
    from loomground_mandate import divergence as mod
    src = inspect.getsource(mod)
    assert "versum" not in src.lower()
    assert "from loomground_solver.ports import EvidenceProvider" in src
...
def test_every_divergence_carries_the_shape_a_brief_reads():
    # The surface that renders these lives in its own package and is deliberately
    # not imported here. What this repo owes it is three fields; what it owes it
    # beyond that is nothing. Testing the contract rather than the integration is
    # what lets either side change without the other being rebuilt.
    ds = _detect(MANDATE, [S("place an order", serves=frozenset({"procure"}))])
    assert ds, "expected something to report"
    for d in ds:
        assert d.kind in KINDS
        assert d.ref and d.why
        assert set(d.to_dict()) == {"kind", "ref", "why"}
